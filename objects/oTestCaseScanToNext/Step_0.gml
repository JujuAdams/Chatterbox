var _check = false;

if (ChatterboxIsWaiting(box))
{
    if (keyboard_check_released(vk_space))
    {
        ChatterboxContinue(box);
        _check = true;
    }
}
else
{
    var _index = undefined;
    if (keyboard_check_released(ord("1"))) _index = 0;
    if (keyboard_check_released(ord("2"))) _index = 1;
    if (keyboard_check_released(ord("3"))) _index = 2;
    if (keyboard_check_released(ord("4"))) _index = 3;
    
    if (_index != undefined)
    {
        ChatterboxSelect(box, _index);
        _check = true;
    }
}

if (_check)
{
    var _metadataArray = ChatterboxGetContentMetadata(box, 0);
    if (is_array(_metadataArray))
    {
        //If you're on a newer version of GameMaker then use `array_contains()` instead of this repeat-loop
        var _found = false;
        var _i = 0;
        repeat(array_length(_metadataArray))
        {
            if (_metadataArray[_i] == "auto")
            {
                _found = true;
                break;
            }
            
            ++_i;
        }
        
        if (_found)
        {
            ChatterboxScanForwards(box);
        }
    }
}
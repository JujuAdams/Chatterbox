...
function __ChatterboxCompile(_in_substring_array, _root_instruction, _hash_prefix)
{
    ...
    while(_s < _substring_count)
    {
        ...
        if (_type == "option")
        {
            ...
            _instruction.optionUUID = string_replace(_hash_prefix, "#", _line);
        }
        else if (_type == "command")
        {
            ...
            switch(_first_word)
            {
                ...
                default:
                    if (_first_word == "")
                    {
                        var _instruction = new __ChatterboxClassInstruction("blank", _line, _indent);
                    }
                    else if ((_first_word == "option") && (_remainder == "split"))
                    {
                        var _instruction = new __ChatterboxClassInstruction("option split", _line, _indent);
                    }

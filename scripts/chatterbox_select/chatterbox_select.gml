...
if (is_real(_selected_index))
{
    ...
    repeat(9999)
    {
        ...
        if (_scan_from_last_wait)
        {
            ...
            {
                if (CHATTERBOX_DEBUG_SELECT) __chatterbox_trace("                    == end inst ", _end_instruction);
        ...
        if (!_break && !_continue)
        {
            switch(_instruction_type)
            {
                ...
                case __CHATTERBOX_VM_ELSEIF:
                    ...
                    if (CHATTERBOX_DEBUG_SELECT) __chatterbox_trace("                    Set _if_state = " + string(_if_state));
                    
                    if (_if_state)
                    {
                        _permit_greater_indent = true;
                        if (CHATTERBOX_DEBUG_SELECT) __chatterbox_trace("                    Set _permit_greater_indent = " + string(_permit_greater_indent));
                    }
                break;
        ...
        if (!_break && !_continue)
        {
            ...
            switch(_instruction_type)
            {
                ...
                case __CHATTERBOX_VM_TEXT:
                    ...
                    if (CHATTERBOX_DEBUG_SELECT) __chatterbox_trace("                    Set _scan_from_text = " + string(_scan_from_text));
                    ...
                    if (CHATTERBOX_DEBUG_SELECT) __chatterbox_trace("                    Set _indent_for_options = " + string(_indent_bottom_limit));

...
function __ChatterboxOnce(_identifier, _node_title, _filename)
{
    ...
    if (string_pos(":", _node_title) > 0)
    {
        ...
        var _key = "once(" + string(_identifier) + "@" + string(_node_title) + ")";
    }
    else
    {
        ...
        var _key = "once(" + string(_identifier) + "@" + string(_filename) + CHATTERBOX_FILENAME_SEPARATOR + string(_node_title) + ")";
    }
